...
export class SelectableGroup extends Component<TSelectableGroupProps> {
  ...
  selectCustomItems = (itemsToSelect: ListOption[]) => {
    const allRegistryItems = Array.from(
      this.registry.values(),
    ) as ListHandlerType[];
    const itemsKeys = itemsToSelect.map((item: ListOption): string => item.key);
    for (const item of allRegistryItems) {
      if (itemsKeys.includes(item.props.item.key)) {
        item.setState({ isSelected: true });
        this.selectedItems.add(item);
      }
    }
  ...
  unselectCustomItems = (itemsToUnselect: ListOption[]) => {
    const allRegistryItems = Array.from(
      this.registry.values(),
    ) as ListHandlerType[];
    const itemsKeys = itemsToUnselect.map(
      (item: ListOption): string => item.key,
    );
    for (const item of allRegistryItems) {
      if (itemsKeys.includes(item.props.item.key)) {
        item.setState({ isSelected: false });
  ...
  mouseUp = (event: Event) => {
    ...
    if (
      ...
      const [selectingItemsReference] = (Array.from(
        this.selectingItems.values(),
      ) as ListHandlerType[]).sort(
        (a: ListHandlerType, b: ListHandlerType) =>
          a.props.item.itemIndex - b.props.item.itemIndex,
      );
      if (selectingItemsReference) {
        this.lastClickedReference =
          selectingItemsReference.props.item.itemIndex;
      }
  ...
  handleClick(evt: any, top: number, left: number) {
    ...
    if (
      ...
      onSelectionFinish!([...this.selectedItems], this.clickedItem, evt);
